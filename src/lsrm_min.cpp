...
//[[Rcpp::export]]
Rcpp::List lsrm_min_cpp(arma::mat data, const int nsample, const int nitem, const int ndim,
  ...
  arma::dmat old_sample_min(nsample, ndim, fill::zeros);
  arma::dmat new_sample_min(nsample, ndim, fill::zeros);
  
  arma::dmat old_item_min(nitem, ndim, fill::zeros);
  arma::dmat new_item_min(nitem, ndim, fill::zeros);
  ...
  for(int iter = 1; iter <= niter; iter++){
    ...
    for(i = 0; i < nitem; i++){
      ...
      old_sample_min.fill(0.0);
      new_sample_min.fill(0.0);
      
      #pragma omp parallel for private(j,k) default(shared)
      for(k = 0; k < nsample; k++){
        for(j = 0; j < ndim; j++){
          old_sample_min(k,j) = std::fabs(old_z(k,j)-old_w(i,j));
          new_sample_min(k,j) = std::fabs(old_z(k,j)-new_w(i,j));
        }
        old_sample_dist(k) = old_sample_min(k,0);
        new_sample_dist(k) = new_sample_min(k,0);
        for(j = 1; j < ndim; j++){
          if(old_sample_min(k,j) < old_sample_dist(k)) old_sample_dist(k) = old_sample_min(k,j);
          if(new_sample_min(k,j) < new_sample_dist(k)) new_sample_dist(k) = new_sample_min(k,j);
        }
        if(data(k,i) == 1.0){
    ...
    for(k = 0; k < nsample; k++){
      ...
      old_item_min.fill(0.0);
      new_item_min.fill(0.0);
      
      #pragma omp parallel for private(i,j) default(shared)
      for(i = 0; i < nitem; i++){
        for(j = 0; j < ndim; j++){
          old_item_min(i,j) = std::fabs(old_z(k,j)-old_w(i,j));
          new_item_min(i,j) = std::fabs(old_z(k,j)-new_w(i,j));
        }
        old_item_dist(i) = old_item_min(i,0);
        new_item_dist(i) = new_item_min(i,0);
        for(j = 1; j < ndim; j++){
          if(old_item_min(i,j) < old_item_dist(i)) old_item_dist(i) = old_item_min(i,j);
          if(new_item_min(i,j) < new_item_dist(i)) new_item_dist(i) = new_item_min(i,j);
        }
        if(data(k,i) == 1.0){
    ...
    for(i = 0; i < nitem; i++){
      ...
      for(k = 0; k < nsample; k++){
        for(j = 0; j < ndim; j++) old_sample_min(k,j) = std::fabs(old_z(k,j)-old_w(i,j));
        old_sample_dist(k) = old_sample_min(k,0);
        for(j = 1; j < ndim; j++)
          if(old_sample_min(k,j) < old_sample_dist(k)) old_sample_dist(k) = old_sample_min(k,j);
        if(data(k,i) == 1.0){
    ...
    for(k = 0; k < nsample; k++){
      ...
      #pragma omp parallel for private(i,j) default(shared)
      for(i = 0; i < nitem; i++){
        for(j = 0; j < ndim; j++) old_item_min(i,j) = std::fabs(old_z(k,j)-old_w(i,j));
        old_item_dist(i) = old_item_min(i,0);
        for(j = 1; j < ndim; j++)
          if(old_item_min(i,j) < old_item_dist(i)) old_item_dist(i) = old_item_min(i,j);
        if(data(k,i) == 1.0){
    ...
    if(iter > nburn && iter % nthin == 0){
      ...
      for(k = 0; k < nsample; k++){
        for(i = 0; i < nitem; i++){
          distance(k,i) = std::fabs(old_z(k,0)-old_w(i,0));
          for(j = 1; j < ndim; j++){
            if(std::fabs(old_z(k,j)-old_w(i,j)) < distance(k,i)) distance(k,i) = std::fabs(old_z(k,j)-old_w(i,j));
          }
