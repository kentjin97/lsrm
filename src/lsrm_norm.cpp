...
//[[Rcpp::export]]
Rcpp::List lsrm_norm_cpp(arma::mat data, const int nsample, const int nitem, const int ndim,
  ...
  for(i = 0; i < nitem; i++) old_w(i,ndim-1) = 0.0;
  ...
  for(int iter = 1; iter <= niter; iter++){
    ...
    if(iter % nprint == 0){
      ...
      printf("%.3f %.3f %.3f %.3f %.3f %.3f\n", old_z(0,0), old_z(0,1), old_z(0,2), old_w(0,0), old_w(0,1), old_w(0,2));
    }
  }
