...
//[[Rcpp::export]]
Rcpp::List lsrm_ms_eq_cpp(arma::cube data, arma::vec nsample, arma::vec nitem, const int ndim,
  ...
  for(s = 0; s < nset; s++){
    old_w.fill(0.0);
    old_w.submat(0,0,nitem(s)-1,ndim-1) = randu<mat>(nitem(s),ndim) - 0.5;
    w_field(s,0) = old_w.submat(0,0,nitem(s)-1,ndim-1);
  }
  ...
  for(int iter = 1; iter <= niter; iter++){
    for(s = 0; s < nset; s++){
      ...
      old_w.submat(0,0,nitem(s)-1,ndim-1) = w_field(s,0);
